...
work_path1 = os.path.dirname(os.path.abspath(__file__))
start = time.time()
...
class TkFileDialogExample(Tkinter.Frame):
    ...
    def __init__(self, root):
        ...
        # taking path from the path_default.txt file - old desing, output is not used currently
#        path_doc = open('path_default.txt', 'r')
#        lib_path_unedited = path_doc.readline()
#        lib_path = lib_path_unedited.replace('\n', '')
#        path_doc.close()
#        print lib_path

        self.plug.set(240)
        self.lp.set(work_path1 + '\LIBRARY')
    ...
    def text_window(self, text1):

        t = Tkinter.Toplevel(self)
        t.wm_title('Parts to replace')
        text_box = Tkinter.Text(t)
        text_box.insert('1.0', text1)
        text_box.pack()
        tkMessageBox.showinfo(title="Parts to replace",
            message="When parts are in CATIA, activate the main product and press OK")
    ...
    def askopenfile(self):
        ...
        f_json = tkFileDialog.askopenfile(parent=root, mode='rb', title='Choose a file')
        f_name = f_json.name
        # adding json export
        if 'json' in f_json.name:
            f_name = json_parsing.parse_ss(f_json.name)
            f_json.close()
            f = open(f_name, 'r')
        else:
            f_json.close()
        ...
        #path_doc_edit = open('path_default.txt', 'w')
        #path_doc_edit.write(path)
        #path_doc_edit.close()
        print path
        global SOW
        SOW = str(self.sp.get())
        ...
        if create_irms:
            if enovia_connected:
                replacedDetails = part_number_collector()
                replacedDetails_str = '\n'.join(replacedDetails)
                self.text_window(replacedDetails_str)
                #root.clipboard_clear()
                #root.clipboard_append(replacedDetails_str)
                replacer()
            reframe_all()
...
def reframe_all():
    ...
    """
    Ecsecutes 'Reframe All' CATIA command
    :return: nothing
    """
...
def irm_builder_constant():
    ...
    # -9 Bin breaks
    if plug == 240:
        bin_breaks = [561, 690 + 120, 894 + 120, 1089 + 120, 1290 + 120, 1401 + 96 + 120, 1560 + 240]
    # -8 Bin breaks
    elif plug == 0:
        bin_breaks = [690, 894, 1089, 1290, 1470, 1560]
    # -10 Bin breaks
    else:
        bin_breaks = [609 + 48, 690 + 240, 894 + 240, 1089 + 240, 1290 + 240, 1401 + 90 + 240, 1618 + 240 + 120 + 40]
...
def instantiate_omf_irm_and_carm(product_in_work, name):
    ...
    if make_carms:
        ...
        carm_instance.add_jd_annotation('01', sta_value_pairs, size, side, 4)
        #carm_instance.access_captures(5)
        carm_instance.add_jd_annotation('02', sta_value_pairs, size, side, 5)
...
def instantiate_omf_nonconstant_irm_and_carm(product_in_work_nonconstant, name):
    ...
    if make_carms:
        ...
        carm_instance.create_jd_vectors(2)
        #carm_instance.access_captures(4)
        carm_instance.add_jd_annotation('01', sta_value_pairs, size, side, arch, 4)
        #carm_instance.access_captures(5)
        carm_instance.add_jd_annotation('02', sta_value_pairs, size, side, arch, 5)
        carm_instance.shift_camera(sta_value_pairs, size)
        #carm_instance.access_captures(1)
        #carm_instance.hide_unhide_captures('unhide', 1)
        carm_instance.activate_top_prod()
...
def instantiate_upper_bin_carm_middle(product_in_work, name, product_for_paste_upr):
    ...
    if make_carms:
        ...
        carm_instance.add_ref_annotation(sta_value_pairs, size, side, 3)
        carm_instance.add_sta_annotation(sta_value_pairs, sta_values_fake, size, side, 3)
        #carm_instance.access_captures(1)
        #carm_instance.hide_unhide_captures('unhide', 1)
        carm_instance.activate_top_prod()
        carm_instance.copy_ref_surface_and_paste(size)
...
def instantiate_carm_lwr(product_inwork, name, product_forpaste_upr, state, first_elem_in_irm):
    ...
    if make_carms:
        ...
        if state == 'final':
            ...
            carm_instance.add_jd_annotation('01', sta_value_pairs, size, side, arch, 6)
            #carm_instance.access_captures(7)
            carm_instance.add_jd_annotation('02', sta_value_pairs, size, side, arch, 7)
            #carm_instance.access_captures(8)
            carm_instance.add_jd_annotation('03', sta_value_pairs, size, side, arch, 8)
            #carm_instance.access_captures(9)
            carm_instance.add_jd_annotation('04', sta_value_pairs, size, side, arch, 9)
            #carm_instance.access_captures(1)
            #carm_instance.hide_unhide_captures('unhide', 1)
            carm_instance.create_jd_vectors(1)
            carm_instance.create_jd_vectors(2)
